...
\subsection{Additional Post-processing}

In addition to extracting scalar quantities from the simulation outputs, several secondary diagnostic plots were generated from the raw data. These plots were used to connect each numerical result to a specific physical interpretation rather than only reporting final values. The equilibrium electric-field profile was used to examine how the depletion-width criterion affects $W$. The ratio $J_{0,\mathrm{sim}}/J_{0,\mathrm{theory}}$ was calculated to make the deviation from ideal diffusion theory explicit across the doping sweep. For the PIN study, the thickness sweep was re-plotted as a multi-metric trend, and the same-thickness PIN versus p--n comparison was summarized across multiple device metrics. These additional plots help identify where analytical formulas are robust and where drift-diffusion simulation gives extra information.
